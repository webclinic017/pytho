...
export const calculateVol = (returns) => {
  const avg = round(returns.reduce((acc, curr) =>
    acc+curr, 0)/returns.length);
  const squaredsum = round(returns.map((v) =>
    Math.pow(v-avg, 2)).reduce((acc, curr) => acc+curr, 0));
...
export const calculateMaxDD = (returns) => {
  ...
  for (let i=0; i < path.length; i++) {
    const currVal = path[i];
    if (currVal > maxVal) {
      maxVal = currVal;
      minVal = maxVal;
    } else if (currVal < minVal) {
      minVal = currVal;
      const tmpDD = (minVal-maxVal)/maxVal;
      if (tmpDD < maxDD) {
        maxDD = tmpDD;
      }
    }
