...
def price_history(request):
    ...
    if coverage_obj_result and len(coverage_obj_result) > 0:
        ...
        return JsonResponse({
            "prices": prices, "country_name": coverage_obj.country_name, 
            "name": coverage_obj.name, "ticker": coverage_obj.ticker,
...
def price_coverage_suggest(request):
    ...
    if security_type:
        return JsonResponse({"coverage": list(Coverage.objects.filter(security_type=security_type, name__icontains=suggest).values())})
    else:
        return JsonResponse({"coverage": []})
...
def price_coverage(request):
    security_type = request.GET.get("security_type", None)
    if security_type:
        return JsonResponse({"coverage": list(Coverage.objects.filter(security_type=security_type).values())})
    else:
        return JsonResponse({"coverage": []})
