...
class FactorSource(DataSource):
    ...
    def find_dates(self, dates: List[pd.Timestamp]) -> DataSource:
        return FactorSource(self.data.loc[dates])

    def find_index(self, start: pd.Timestamp, end: pd.Timestamp) -> DataSource:
...
class InvestPySource(DataSource):
    ...
    def find_dates(self, dates) -> DataSource:
        return InvestPySource(self.data.loc[dates])

    def find_index(self, start, end) -> DataSource:
