...
class TestFixedSignalBackTestWithPriceAPI(TestCase):
    ...
    def test_that_errors_from_investpy_are_handled(self, mock_price, mock_coverage):
        ...
        self.assertRaises(BackTestUnusableInputException, FixedSignalBackTestWithPriceAPI, [1,2], [0.5, 0.5]) 

        mock1.get.side_effect = ConnectionError('foo')
        self.assertRaises(ConnectionError, FixedSignalBackTestWithPriceAPI, [1,2], [0.5, 0.5]) 
    ...
    @patch("helpers.backtest.backtest.Coverage")
    @patch("helpers.backtest.backtest.prices.PriceAPIRequests")
    def test_that_throws_error_with_valid_but_bad_input(self, mock_price, mock_coverage):
        mock = Mock()
        ...
        self.assertRaises(BackTestUnusableInputException, FixedSignalBackTestWithPriceAPI, assets, weights) 
    ...
    @patch("helpers.backtest.backtest.Coverage")
    @patch("helpers.backtest.backtest.prices.PriceAPIRequests")
    def test_that_throws_error_when_missing_assets_or_weights(self, mock_price, mock_coverage):
        mock = Mock()
        ...
        assets = []
        weights = []
        self.assertRaises(BackTestInvalidInputException, FixedSignalBackTestWithPriceAPI, assets, weights) 

        assets1 = [1,2]
        weights1 = []
        self.assertRaises(BackTestInvalidInputException, FixedSignalBackTestWithPriceAPI, assets1, weights1)

        assets2 = []
        weights2 = [0.5, 0.5]
        self.assertRaises(BackTestInvalidInputException, FixedSignalBackTestWithPriceAPI, assets2, weights2)

        assets3 = [1]
        weights3 = [0.5, 0.5]
        self.assertRaises(BackTestInvalidInputException, FixedSignalBackTestWithPriceAPI, assets3, weights3)
        return
